...
## 前言
⚠️本插件主要由AI语言模型ChatGPT辅助编写代码,并由我进行审查、测试和修改。虽然我们已尽最大努力确保插件的可用性和稳定性,但不能完全保证不存在潜在的bug或兼容性问题。同时,这个插件是基于我个人在"求生之路2"模组制作中的工作流程和习惯开发的,我们尽量考虑了通用性和可扩展性,但可能无法完全满足所有模组制作者的需求。
1
⚠️如果您在使用过程中遇到任何问题或有任何改进建议,欢迎在Github仓库中提交Issue,我们会认真查阅并努力满足大家的需求。

...
- 接着讲对齐骨骼后的操作，用对齐骨骼创建一批骨骼约束后，先设置一次静置姿势定位姿态，然后移除骨骼约束完成对齐操作的收尾。
- 从这里开始可以分为两种嫁接流程，一种是保留原骨骼重新设置父子级关系。另一种是先把自定义骨骼重命名为标准官骨命名，然后利用cats插件把同名骨骼合并进官方骨架中。
...
- 先演示第一种功能的嫁接，这个功能有两个主要功能和一个特殊功能。
  1. 当选中某一些骨骼时，只对这些骨骼进行嫁接操作。
  2. 当没有选中骨骼时，默认对全体骨骼进行嫁接操作。
  3. 特别的，当多选的骨骼中包含官骨，则其他骨骼不管有没有在映射关系中，均会成为这根官骨的子级。
- 原理依旧是利用了上面设置过的骨骼映射关系，同时还加入了一个距离判定，只要有几乎完全重合的骨骼，就会在其中设置父子级关系。
